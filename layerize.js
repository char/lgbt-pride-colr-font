...
if (fontName == undefined) {
    ...
    console.error("### Usage: node " + process.argv[1] + " assets-dir extras-dir build-dir font-name");
    return;
}
...
rmdir(targetDir, function () {
    ...
    for (const fileName of assets) {
        if (fileName.match(/[0-9a-fA-F]+\.svg/)) {
            console.log(fileName);
            data = fs.readFileSync(assetsDir + "/" + fileName);
