...
$(FINAL_TARGET) : $(RAW_FONT) $(OT_SOURCE)
	...
	$(PERL) -i -e 'my $$ps = 0;' \
	        ...
	        -e '  s/Charlotte Pride/CharlottePride/ if $$ps;' \
	        -e '  print;' \
	        -e '}' $(RAW_FONT).names
...
$(CODEPOINTS) $(OT_SOURCE) : $(LAYERIZE) $(OVERRIDE_DIR) $(EXTRA_DIR)
	$(NODE) $(LAYERIZE) $(ASSETS_DIR) $(EXTRA_DIR) $(BUILD_DIR) $(FONT_NAME)
